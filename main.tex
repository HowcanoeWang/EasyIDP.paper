...
Applying high-throughput phenotyping by 3D reconstruction in agriculture provides an advanced and efficient method for managing and monitoring crops. The \acrfull*{sfm} 3D reconstruction, applicable to price friendly RGB digital cameras, has been widely used to implement this task. However, producing good quality products, including \acrfull*{dom}, \acrfull*{dsm}, and \acrfull*{pcd}, is quite computation intensive and time-consuming, and many studies isolated analyze those outputs without linking to raw images which has abundant first-hand information. Meanwhile, directly applying those outputs to deep learning framework is still difficult.

\parttitle{Results}
In this study, a pure python package called EasyRIC (easy reconstruction image converter) was developed to linking the outputs of \acrshort*{sfm} products with original photos. The Lotus (\textit{Nelumbo nucifera}) breeding field were used  to demonstrate the following functions: 1) preparing DOM and given \acrfull*{roi} to deep learning required size and annotation format. 2) transforming ROI from \acrshort*{sfm} products to high quality raw images. 3) evaluating the accuracy of previous transformation.

\parttitle{Conclusions}
This python package shows the acceptable accuracy and potential to integrate the high-quality original images with \acrshort*{sfm} produced products, which makes producing time-consuming ultra-high quality products unnecessary to some content. Also, this tool has the potential to significantly decrease the workload of deep learning training data augmentation linking back to high quality raw images with abundant scale, rotation, position variance.
...
The world population is expected to increase to 9.6 billion in 2050 and 10.9 billion in 2100 \cite{gerland_world_2014}, however, to ensure the food security of human, the current yield gains increase (average 1.2\% annual) is needed to be doubled by 2050 \cite{ray_yield_2013}. Based on the breeder's equation \cite{eberhart_factors_1970, cobb_enhancing_2019}, the annual genetic gain ($\Delta G$) equals to $\dfrac{\sigma_{a} \cdot i \cdot r}{L}$, where $\sigma_a$ is the genetic variation within the population, $i$ is the selection intensity, $r$ is the selection accuracy, and $L$ number of years per cycle. Currently, the $i$ and $r$ is closely related to the crop management strategies, that is the throughput and accuracy of field data collection. The traditional in field mensuration manually is not only time-consuming and labor intensive, but also highly related the field experience of investigators, which caused unavoidable subjective to measured traits. At the meanwhile, limited by the traditional measuring tools, the simple linear geometry traits such as width, length, height, angle, or flowering time records are often obtained with heavy workload.

% Common high-throughput phenotyping technologies -> traits.
With the development of sensing and chemical analyzing techniques, the  contemporary plant phenotyping studies extends the traits to three main classes: component, functional and structural. The component traits include the contents of nitrogen, sugar, protein, and others. While the functional traits includes photosynthesis efficiency \cite{song_optimal_2013}, resistance to stress (e.g. drought \cite{araus_phenotyping_2012, xiang_robotic_2020}), and others, based on novel sensors like fluorescence \cite{krause_chlorophyll_1991} or hyperspectral \cite{yendrek_high-throughput_2017}. For structural traits, mainly about the geometry measurements of plant structure, such as plant height \cite{ten_harkel_biomass_2019}, leaf length, angles, leaf area index \cite{zermas_3d_2020}, projected leaf area \cite{guo_easypcc:_2017}, shoot growth \cite{li_measuring_2020}, skeleton structure \cite{wu_accurate_2019}, and biomass \cite{ten_harkel_biomass_2019, jin_non-destructive_2020}, either by 2D images analysis or by 3D reconstruction.

% categories of 3D reconstruction.
Compared with image analysis on 2D images, the 3D reconstruction technique keeps the major structural information and reduces the impact of occlusion in the step of projecting real world to camera sensors. As Paulus \cite{paulus_measuring_2019} reviewed, currently popular 3D reconstruction technique includes laser triangulation, structured light, time of flight (ToF camera or LiDAR), light field measuring, and structure from motion (SfM). Except SfM technique is the cheapest using common digital camera, other techniques require expensive special instruments, making this technique has great potential for widely practical use in agricultural phenotyping \cite{jay_-field_2015,sun_three-dimensional_2020, zhu_quantification_2020}.

% SfM background
The SfM algorithm was oriented in the computer vision area \cite{ullman_interpretation_1979}, assuming the modeling objects are static rigid body under uniform light condition with only the camera moving, and applied to the geoscience widely which fits well with this assumption \cite{westoby_structure--motion_2012, mu_mcget:_2018}. The full process could be extended to SfM step and multi-view stereo (MVS) step. In the SfM step, the camera internal information such as actual sensor size, photo resolution, focal length could be extracted from image metadata, then the feature extraction algorithm is used to extract the feature points on each image, and a feature matching algorithm based on \acrfull*{kd-tree} and Euclidean distance is applied to filter image pairs with enough number of matching feature points. For each image pairs, the Epipolar geometry and optimized by \acrfull*{ransac} are used to get the relative camera position, and iterate getting the camera motion path. The bundle adjustment \cite{triggs_bundle_1999} algorithm is applied to calibrating the projection errors among feature points of all images, and finally get the external camera parameters (position and rotation) and sparse feature point cloud. Also, previous image alignment is continuing optimized by other studies \cite{lowe_distinctive_2004} when integrated for commercial software. For MVS steps, all the pixels in raw images associated with feature points are used to interpolating the wholes in spare feature point cloud after noise removal to get a dense \acrfull*{pcd}, for more detail, please refer this book \cite{hartley_multiple_2000} and this review \cite{snavely_scene_2010}. Lastly, the raw images could be masked over the dense point cloud by obtained geometry, and export to GIS products such as \acrfull*{dsm} and \acrfull*{dom}.

% how the outputs been used differently indoor and outdoor
The application of SfM could be split to indoor and outdoor. For indoor application, the DOM and DSM are not the mandatory outputs compared with PCD. Many indoor studies proved the feasibility of \acrshort*{sfm} technology to build PCD of individual crops with good quality \cite{wang_maize_2019, rossi_performances_2020}, and the visually close to that obtained by LiDAR \cite{thapa_novel_2018} and RGB-Depth camera \cite{martinez-guanter_low-cost_2019}. Several studies focused on developing algorithms to classify or segment point clouds, and then calculate geometry traits from previous results \cite{ziamtsov_machine_2019, artzet_phenomenal:_2019}. And for outdoor experimental fields application, though PCD is also an important data source for crop modeling and traits extraction \cite{jay_-field_2015, sun_three-dimensional_2020, zhu_quantification_2020}, the DOM and DSM are also required for plot management and simplify the crop analyzing difficulties \cite{sun_-field_2018, tresch_easy_2019, chen_grid_2020}. Aligning the SfM outputs of different time also provide the ability for time-series analysis \cite{dong_4d_2017, han_clustering_2018}.

% The current challenges that using DOM PCD and DSM
Though the feasibility of applying SfM products into agriculture phenotyping have been validated by many studies, there are still several pain points exist. 

Firstly, the quality of DOM can not reach the same quality as original images. In the outdoor environment, the uneven light condition and plant leaf movement caused by wind challenges the assumption of SfM-MVS, and it could result in the 'ghost effect' on DOM and cause the over-estimation of ground coverage than that on raw images takes over 79\% of plots \cite{duan_comparison_2017}. Meanwhile, though SfM-MVS software provides the ultra-high quality option in MVS stages, interpolating thousands of raw images of experimental field at ultra-high quality (density) is computation intensive and time-consuming, it is not feasible to apply high-throughput phenotyping for a large field area. 

Secondly, the size of outputs is not so convenient to use, especially for deep learning applications. Applying deep learning framework to 2D images for object classification or segmentation is now a popular research area in agriculture \cite{zhou_fast_2020,feng_evaluation_2020, desai_automatic_2019}, however, limited by the huge memory consumption during convolution, the input images size is often limited to small size, while the original DOM size is hundreds of times larger than that. Additionally, the DOM and PCD in the field often have unused background, and gathering them into one file making data analyzing more complicated. These summarized to one point: how to clipping SfM-MVS outputs to interested parts conveniently? Meanwhile, how to transform GIS often used format to deep learning required annotation format (*.json) after clipping is another challenge. 

Thirdly, the value of raw images is often ignored during recent studies. The SfM step naturally build a geometry relationship between raw images and GIS products. Applying deep learning framework on unstructured \acrfull*{pcd} is significantly different with on structured matrix image data. Though the point cloud deep learning exists (e.g. PointNet \cite{qi_pointnet_2017}) and some studies applying adapted deep learning framework on good quality point cloud data obtained by LiDAR directly \cite{jin_deep_2018, jin_stemleaf_2019, jin_non-destructive_2020}, Van de Zedde et al., \cite{van_de_zedde_plant-part_2019} showed the potential of projecting deep learning application results on raw images to \acrfull*{pcd} based on the geometry relationship between raw images and point clouds. Meanwhile, the raw images collected from different positions result in abundant view angle, rotations, and scales, are naturally a good material for data augmentation, a current deep learning challenge can't be solved with light workload.

The objective of this study, is developing an easy-to-use software package, to solve previous three main plant points, providing functions of: 1) making DOM and \acrfull*{roi} to deep learning acceptable format, based on the SfM-MVS outputs clipping function for optimized data management. 2) connecting and transforming the ROI among SfM-MVS outputs and raw images, which showed the potential for deep learning training data augmentation. 3) testing the accuracy of previous transformation by a case study.
...
The EasyRIC is a python package without any \acrfull*{gui}, please refer to the package documentation for detailed usage. Instead of introducing how the package is used, this section demonstrate how the calculation algorithm implemented.
...
In this case study, after clipping out the \acrfull*{roi} of each pond, two heights are calculated. The one is the 5 percentile mean z value ($mean(Z_{p5})$), equals to the mean z values of all the points smaller that 5 percentile threshold, used for catching the elevation (height) of water surface to define the z value of ROI. The other is the 95 percentile mean z value ($mean(Z_{p95})$), equals to the mean of all point z values which bigger than 95 percentile threshold, used for catching the top of lotus leaves. Also, a ground region was selected manually (The middle corridor in Figure \ref{fig:map}.b where scale bar placed), the mean Z value of this region ($mean(Z)_{ground}$) was used as the ground elevation. The lotus height equals to $mean(Z_{p95}) - mean(Z)_{ground}$.
...
When it comes to Z axis, though the height of lotus showed a general trend of increasing, the time 20170531, 20170612, and 20170718 showed an abnormal trend which height is significantly greater than later days. This should be effected by the quality of \acrfull*{sfm} processing. By visual checking, in these days, the 3D reconstructed ground had slightly leaned from west to east, making the $mean(Z)_{ground}$ wasn't the actual ground height near that pond and caused this abnormal height. What's more, the z value (elevation) of each time did not be calibrated the same as XY plane, hence the $mean(Z)_ground$ variate differently along the timeline, the deviation could reach 0.3 m which is unacceptable compared with crop height. In the future, the \acrshort*{gcp} for z axis should also taking into consideration, for example, a visible box or pole could be set in the field. How to evaluating the quality of 3D reconstruction and the adjustment for the 3D reconstruction workflow to fit the characteristic of agricultural should also be conducted.

\subsection*{Use case 3: Transformation ROI on raw images}
% why reverse calculation: the resolution problem
Though the \acrfull*{sfm} software provide options for the quality (density) of MVS stage, the highest quality could provide a ultra high resolution of outputs. However, this processing is unacceptable computation intensive and time consuming for high-throughput agriculture, meanwhile, a study showed the ground coverage estimated from DOM is greater than that from raw images took over 79\% of plots due to the ghosting effects caused by wind and cloud \cite{duan_comparison_2017}. For 2D traits analysis, e.g. ground coverage, leaf projection area, or object detection, marking \acrfull*{roi} on fast produced low quality DOM and linking them back to raw images could save a plenty of time without losing the accuracy, this technique named ROI transformation in this study, or called reverse calculation in previous studies \cite{duan_comparison_2017,tresch_easy_2019}. The demo of ROI transformation from DOM and PCD on some raw images is shown in Figure \ref{fig:roi2dxm}.e.

% how it performs (IoU accuracy analysis)
The results of performance evaluation is shown in Figure \ref{fig:dist}. For pond N2W5 and S2W4, most the IoU values are over 90\%, surprisingly, for the most simple and sparse pond N3E6, though the IoU values are greater than 75\%, the performance is moderate. The reason is the ROI z value (elevation, $mean(Z_{p5})$) automatic calculation. For manually marking, the reference is the edge of pond, which means the manual ROI elevation is the pond edge (the blue broken line in the second row of Figure \ref{fig:dist}), while the elevation auto-selected is the red solid line in Figure \ref{fig:dist}. The plot N3E6 had almost 25 cm differences between two elevations, mainly caused by the transparent water, the 3D reconstruction build the bed mud rather than water surface for this pond. This shows the ROI height selection is of great importance for the transform accuracy. Another trend in this figure is the IoU decreasing with the distance from ROI center to photo center increasing. It means the position of ROI on raw images (the view angle where taking raw photo) also reflect the transform accuracy. 

Figure \ref{fig:ht_diff} shows how the ROI elevation selection and ROI position on raw images effects transformation accuracy. Three different heights (bottom $mean(Z_{p5})$, middle $mean(Z)$, and top $mean(Z_{p95})$) were selected as ROI elevation respectively (Figure \ref{fig:ht_diff}.a), on different images, the deviation variate. The closer to the raw image center, the less effects of height selection, which is highly related to the view angle effects. And previous Figure \ref{fig:dist} also shows, once the pixel distance to image center smaller than 800 pixels (size of image is 4608$\times$3456 pixels), the moderate performance pond N3E6 could achieve IoU greater than 90\%. Hence, to minimize the effects of unsuitable ROI elevation selection, it is recommended to choose raw images where the ROI locates in the center as first priority. Based on this finding, for all 112 ponds, the minimum distance raw image of each pond were selected and marked expected ROI manually, the distribution of accuracy indicators is shown in Figure \ref{fig:iou_all}, the peaks are around 98\% while
the minimum value still greater than 90\%. Considering the difficulties in manual marking in some pond where all corners are covered by leaves and unidentical (See Additional file 1, Figure S1), the ROI transformation accuracy has been validated acceptable.
...
pass collect data? thanks Yulin Duan for coming out the idea of DOM clipping?
