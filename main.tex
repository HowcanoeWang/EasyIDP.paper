...
% maybe useful, add citations to this paragraph. 
Using 3D reconstruction by images collected from price friendly digital camera or drones to build plant geometry models and extract traits based on the outputs is a hot topic in agriculture phenotyping studies in recent years. Many of them paid full attention to the traits calculation algorithms, assuming the default setting of \acrshort*{sfm} software could produce trustable outputs, and the evaluation are mainly based on the comparison between the traits measured in field and those calculated by algorithms from 3D reconstruction outputs. 

Para5: 4D time series analysis demand and point cloud clipping
see \cite{han_clustering_2018} or search "time-series" in Zotero.

Para6: Training Data crisis

Para7: Objectives

extra: previous studies \cite{duan_comparison_2017, tresch_easy_2019} also called reverse calculation, but never integrate them into an widely applicable tools. previous study also use pmatrix, but not like this study so systematically
...
pass thanks Yulin Duan for coming out the idea of DOM clipping?
