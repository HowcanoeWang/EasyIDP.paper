...
The geometry of transforming \acrfull*{roi} on one image to another image is shown in Figure \ref{fig:workflow}.c-1. Then, transforming a polygon can be simplified to repeat transforming each polygon corner point to another photo. In this geometry, there are two coordinates, one is the real world geographic coordinate ($xyz_{geo}$, unit is $m$), the other is the raw image pixel coordinate ($xy_{pix}$, unit is pixel). Assuming there are two images, the first image ($raw_1$) is where we use annotation software (e.g. LabelMe) marked ROI, the other image ($raw_2$) is the target image we want this ROI transformed to. The $K_1$ in $xy_{pix}$ coordinate is one polygon corner point on $raw_1$, this point related to the point ($K'$) in real world, and this point is record as $K_2$ on $raw_2$. The $r_i (\omega_i, \varphi_i, \kappa_i)$ represents the camera rotation angles \cite{pix4d_documentation_yaw_2020}, $p_i$ represents the camera position in real world system ($xyz_{geo}$), $f$ is the camera focal length in $mm$, $w$ and $l$ represents the camera sensor width and length in $mm$, respectively. All previous photo related parameters could be obtained in *.cam files.

As the Pix4D document demonstrated \cite{pix4d_documentation_how_2020}, the transformation from real world point $K' (X, Y, Z)$ to image pixel coordinate $K_i (x_{k_i}, y_{k_i})$ is:

\begin{equation}
  \begin{cases}
    x_{k_1} = - f \cdot \dfrac{X'}{Z'} + x_{center} \\
    y_{k_2} = - f \cdot \dfrac{Y'}{Z'} + y_{center}
  \end{cases}
\end{equation}

where, $f$ is camera focal length, $x_{center}$ and $y_{center}$ is the image center pixel coordinate ($xy_{pix}$), width (horizontal) and length (vertical), respectively. $(X', Y', Z')$ can be calculated by:
...
where, $\mathbf{R_i}$ is the rotation matrix of camera parameters can be calculated by $r_i (\omega_i, \varphi_i, \kappa_i)$ \cite{pix4d_documentation_how_2020}:
...
However, previous K' (dim=3) to pixel $k_1$ (dim=2) is a dimension reduction step with Z information loss is not reversible. Transforming from given pixel coordinate $(x_{k1}, y_{k1})$ back to real world (revert Equation 2 and 1) could only get the equation with $Z_1'$ as parametric, which means without knowing the exact value of $Z_1'$:
...
It will produce a line rather than a specific point in the real world as well as on the other image, in this case, the most accurate method is marking the ROI on two images, and then calculate their route intersection to get the exact $Z'$ value of $K'$ in real world, then the follow Equation 1 and 2 to calculate the pixel coordinate on the third image. 
