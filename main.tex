...
\author[
  addressref={minarg, caas}
]{\inits{YD}\fnm{Yulin} \snm{Duan}}
\author[
  addressref={minarg, caas}
]{\inits{YS}\fnm{Yun} \snm{Shi}}
\author[
  addressref={isas, plp}
]{\inits{YK}\fnm{Yoichiro} \snm{Kato}}
\author[
  addressref={isas, nau}
]{\inits{SN}\fnm{Seish} \snm{Ninomiya}}
\author[
  addressref={isas},
  corref={isas}, 
...
\address[id=minarg]{
  \orgname{Key Laboratory of Agricultural Remote Sensing, Ministry of Agriculture},
  \postcode{100081},
  \city{Beijing},
  \cny{China}
}
\address[id=caas]{
  \orgname{Institute of Agricultural Resources and Regional Planning, Chinese Academy of Agricultural Sciences},
  \postcode{100081},
  \city{Beijing},
  \cny{China}
}
\address[id=plp]{
  \orgname{International Rice Research Institute},
  \city{Metro Manila},
  \cny{the Philippines}
}
\address[id=nau]{
  \orgname{Plant Phenomics Research Center, Jiangsu Collaborative Innovation Center for Modern Crop Production,Nanjing Agricultural University},
  \postcode{210095},
  \city{Nanjing},
  \cny{China}
}
...
The 3D based high-throughput phenotyping makes the task of managing and monitoring crops more efficient. The \acrfull*{sfm}, applicable to common RGB digital cameras, has been widely used and can be implemented by many commercial and open-source tools. By using such tools, several outputs like \acrfull*{dom}, \acrfull*{dsm}, and \acrfull*{pcd} can be generated. However, there is a gap from these outputs to 3D plant phenotyping. For example, calculating plant height and canopy ground cover requires the segmentation of each plot from whole DOM or DSM or original images. These intermediate processing is time-consuming but no easy-to-use tools are available to the best of our knowledge. 

\parttitle{Results}
In this study, a software package called EasyIDP (easy intermediate data processor) was developed to link the outputs of \acrshort*{sfm} products with 3D based plant phenotyping. A Lotus (\textit{Nelumbo nucifera}) breeding field was used to demonstrate the following points: 1) clipping (segmenting) \acrshort*{sfm} products according to given plot boundary or \acrfull*{roi}. 2) transforming \acrshort*{roi} on the \acrshort*{sfm} products to high quality raw images to assist object detection. 3) evaluating the accuracy of previous transformation by manual annotation.

\parttitle{Conclusions}
The proposed intermediate data processing tool shows an acceptable accuracy and potential to process those outputs of \acrshort*{sfm}. By using the EasyIDP, a bridge between \acrshort*{sfm} software outputs and plant phenotyping could be built conveniently.
